...
def main():
    ...
    conv_layer = Conv2D(8,(3,3),activation = "relu")(input_layer)
    pool_layer = MaxPooling2D(pool_size=(2,2), strides=(2,2))(conv_layer)
    conv_layer_2 = Conv2D(16,(3,3),activation = "relu")(pool_layer)
    max_layer_2 = MaxPooling2D((2,2),(1,1))(conv_layer_2)
    flat_layer = Flatten()(max_layer_2)
    dense_layer = Dense(64)(flat_layer)
    dropout = Dropout(0.5)(dense_layer)
    output = Dense(num_classes, activation = "softmax")(dropout)
    output_2 = Dense(num_classes, activation='softmax')(dropout)
    model= Model(input_layer,[output,output_2])

    model.compile(loss='categorical_crossentropy',optimizer='sgd', lr=0.1, metrics=['accuracy'], loss_weights=[0.5, 0.5])
	#==================== Fetch Data and Fit ===================#
