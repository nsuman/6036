...
def main():
    ...
    dense_layer = Dense(64,activation = "sigmoid")(flat_layer)
    dense_layer_2 = Dense(64,activation = "sigmoid")(dense_layer)
    output = Dense(num_classes, activation = "softmax")(dense_layer_2)
    output_2 = Dense(num_classes, activation='softmax')(dense_layer_2)
    model= Model(input_layer,[output,output_2])
    model.compile(loss='categorical_crossentropy',optimizer='adadelta',  metrics=['accuracy'], loss_weights=[0.5, 0.5])
